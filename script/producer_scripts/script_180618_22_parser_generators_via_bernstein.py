...
def stream_for_sync_via_stream(dcts):
    ...
    for dct in dcts:
        # #[#410.M] how sparseness (holes) must be filled lost at #history-A.4
...
class _open_raw_traversal_stream:  # #[#410.F] class as context manager
...
def _main(lines, do_field_names, listener):
    ...
    from kiss_rdb.storage_adapters_.markdown_table.magnetics_.open_traversal_request_via_path import (  # noqa: E501
            MARKDOWN_TABLE_SCANNER)

    scn = MARKDOWN_TABLE_SCANNER(
...
if __name__ == '__main__':
    ...
    exit(cheap_arg_parse(
            ...
            formal_parameters=(
                ...
                ('-s', '--for-sync',
                 'translate to a stream suitable for use in [#407] syncing'),
            ),
